...
const carouselPhotos = photos.slice(0, 2)
const loveEmojis = ['💍', '💖', '🥰', '💐', '😂']
...
function App() {
  ...
  const [activePortrait, setActivePortrait] = useState<number | null>(null)
  ...
  const galleryNextImageRef = useRef<HTMLImageElement>(null)
  const galleryCaptionRef = useRef<HTMLSpanElement>(null)
  const galleryNextCaptionRef = useRef<HTMLSpanElement>(null)
  ...
  const playStory = () => {
    ...
    setOpenEpisodes([])
    setOpenEpisode(null)
    window.setTimeout(() => setStoryStage(2), 700)
  }
  ...
  const addToCalendar = () => {
    ...
    calendarUrl.searchParams.set('text', 'Akad Fani dan Ella')
    ...
    calendarUrl.searchParams.set('location', 'Kediaman Mempelai Wanita, Pancurendang RT 04 RW 07 Ajibarang Banyumas')
  ...
  const selectPhoto = (index: number, direction = index > activePhoto ? 1 : -1) => {
    ...
    const nextCaption = galleryNextCaptionRef.current
    const nextImage = galleryNextImageRef.current
    if (!next || !image || !nextImage || !background || !caption || !nextCaption || galleryAnimatingRef.current || index === activePhoto) return
    galleryAnimatingRef.current = true
    gsap.killTweensOf([image, nextImage, background, caption, nextCaption])
    nextImage.src = next.src
    nextCaption.textContent = next.label
    gsap.set(nextImage, { x: 60 * direction, scale: 1.06, opacity: 0 })
    gsap.set(nextCaption, { x: 60 * direction, opacity: 0 })
    const timeline = gsap.timeline({ onComplete: () => { setActivePhoto(index); requestAnimationFrame(() => { gsap.set(image, { x: 0, scale: 1, opacity: 1 }); gsap.set(nextImage, { x: 60 * direction, scale: 1.06, opacity: 0 }); gsap.set(caption, { x: 0, opacity: 1 }); gsap.set(nextCaption, { x: 60 * direction, opacity: 0 }); galleryAnimatingRef.current = false }) } })
    timeline.to([image, caption], { x: -60 * direction, scale: .96, opacity: 0, duration: .7, ease: 'power3.inOut' })
      ...
      .fromTo(nextCaption, { x: 60 * direction, opacity: 0 }, { x: 0, opacity: 1, duration: .9, ease: 'power3.out' }, '-=.9')
  ...
  return (
    <main className="app-shell">
      <header className="topbar">
        <div className="brand"><i /> WEDDINGFLIX</div>
        <button className={`sound-button ${musicPlaying ? 'is-playing' : 'is-paused'}`} onClick={toggleMusic} aria-label={musicPlaying ? 'Jeda musik' : 'Putar musik'}><span className="music-island"><i /><span><b>Overnight</b><small>Kita Lewati Berdua · Wedding Band</small></span></span><span className="music-cassette" aria-hidden="true"><span className="cassette-reel cassette-reel-left" /><span className="cassette-reel cassette-reel-right" /><span className="cassette-label">F&E</span></span></button>
      </header>
      ...
      {tab === 'home' && <>
        <section className="hero-panel">
          <div className="hero-copy">
            ...
            <div className="hero-actions"><button className="primary-button" onClick={() => selectTab('story')}>▶ Cerita</button></div>
          </div>
        </section>
