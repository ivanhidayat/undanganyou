...
function App() {
  ...
  const selectPhoto = (index: number, direction = index > activePhoto ? 1 : -1) => {
    ...
    if (!next || !image || !nextImage || !background || galleryAnimatingRef.current || index === activePhoto) return
    galleryAnimatingRef.current = true
    gsap.killTweensOf([image, nextImage, background])
    ...
    timeline.to(image, { x: -60 * direction, scale: .96, opacity: 0, duration: .7, ease: 'power3.inOut' })
